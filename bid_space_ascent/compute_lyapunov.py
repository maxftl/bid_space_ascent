...
from lyapunov_utils import add_lyapunov_constraints, get_parameter_values
import logging
import numpyencoder
import json
...
def compute_lyapunov(n, filename, max_degree = 2):
    ...
    grad_utility_f = utility_calculator.getUtilityGradient(original_f, original_g)
    grad_utility_g = utility_calculator.getUtilityGradient(original_g, original_f)
    ...
    lyapunov_coeff_substitution = list(zip(
         lyapunov_coefficients.flatten().tolist(),
         lyapunov_coeff_values.flatten().tolist()
    ))
    lyapunov_function = lyapunov_info['lyapunov_function'].subs(lyapunov_coeff_substitution).simplify()
    lyapunov_info['solution'] = lyapunov_function

    with open(filename, "w") as outfile:
        json.dump(lyapunov_info, outfile, cls=numpyencoder.NumpyEncoder)


if __name__ == 'main':
     compute_lyapunov(2, 'results_test_2.json')